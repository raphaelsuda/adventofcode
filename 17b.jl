initial_config(data) =  Set((x, y, 0, 0) for x in 1:size(data, 2), y in 1:size(data, 1) if data[y,x] == '#')

get_neighbors((x, y, z, w)) = Set((i, j, k, l) for i in x-1:x+1, j in y-1:y+1, k in z-1:z+1, l in w-1:w+1 if (i, j, k, l) != (x, y, z, w))
...
function solve17b(data, cycles=6)
